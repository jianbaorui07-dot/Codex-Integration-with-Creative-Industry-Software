...
const elements = {
  connection: document.querySelector("#connection"), host: document.querySelector("#host"),
  document: document.querySelector("#document"), selection: document.querySelector("#selection"),
  synced: document.querySelector("#synced"), message: document.querySelector("#message"),
};
...
function onStatus(status, detail = {}) {
  ...
  if (status === "synced") {
    ...
    elements.document.textContent = detail.document?.name || "无活动文档";
    elements.selection.textContent = String(detail.selection?.length || 0);
    elements.synced.textContent = new Date(detail.at).toLocaleTimeString();
    elements.message.textContent = `对象 ${detail.document?.page_items || 0} · 图层 ${detail.layers?.length || 0} · 画板 ${detail.artboards?.length || 0}`;
  } else if (status.endsWith("error")) elements.message.textContent = detail.message || status;
}
