...
| ComfyUI | 查看节点图、队列、浏览器 UI 报错、预览图状态 | `comfyui.system_probe`、`comfyui.queue_snapshot`、`comfyui.progress_monitor`、`comfyui.workflow_validate`、`comfy.workflow_lifecycle_summary` | GUI 负责视觉检查；MCP 负责 API probe、脱敏队列、单调实时进度和 workflow 校验，不返回预览图或错误正文 |
