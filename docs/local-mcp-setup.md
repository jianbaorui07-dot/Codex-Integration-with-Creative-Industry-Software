...
3. `comfyui.system_probe` / `comfyui.queue_snapshot` / `comfyui.progress_monitor` / `comfyui.workflow_validate` / `comfy.workflow_lifecycle_summary` 已实现；下一步做断线恢复、受控 cancel 和 queue payload dry-run。
