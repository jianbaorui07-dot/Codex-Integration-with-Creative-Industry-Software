# Amara UE Agent Framework

Amara UE Agent Framework is an Unreal Engine experiment for a local-first 3D agent workflow.
...
This repository does not present that full vision as finished. The current package preserves the first local UE 5.2.1 evidence from the `codex接入UE` project: an Amara-prefixed sandbox, UE assets, offline task runner scripts, checkpoints, safety reports, performance evidence, and the stopped Remote Control/MCP handoff plan.

## Repository Package

The packaged artifact is here:

- `examples/unreal_amara_agent/`

It contains:

- `ue_project/Content/Amara/` - Unreal `.uasset` and `.umap` assets for the Amara sandbox content.
...
- `legacy_pre_audit/` - earlier UE pre-audit scripts and evidence used before the Amara package was created.
...
- Amara is a project prefix, not an external program.
- The editable Amara copy produced two maps: `M_AmaraLab` and `M_AmaraBlockoutSandbox`.
- The sandbox reached roughly 42 Amara-managed actors while staying below the 60 actor limit used in the safety gate.
- Existing Amara resources include offline task runner, city mood controller, explorer pawn, control desk, materials, maps, checkpoints, and handoff reports.
...
The Amara package follows the same local-first boundary as StarBridge:
