...
| Illustrator | discovery, planning, execution, validation, evidence, cleanup | safe status/document shape, `illustrator.preflight` on sanitized summaries | `illustrator.color_vectorize_plan` / `validate` / `compare` / `repair_plan`；guarded native Image Trace execute；headless color raster→verified editable SVG fallback | explicit-confirmation bounded repair_plan → execute → compare orchestration；authorized public desktop E2E；VTracer headless backend；final SVG reraster visual-diff gate | native compare reports sanitized PNG metrics but public CI does not prove desktop execution；headless trace proves parseable non-empty paths, zero embedded raster, bytes/SHA-256/path/color counts and rollback | headless only writes ignored `examples/output/illustrator/trace-practice`；native execute requires dual confirmation and sandbox output | headless synthetic color/topology generation on Ubuntu/Windows；native route limited to dry-run/schema/rejection tests | Only native execute needs live Illustrator；headless needs no app | color-vector routes accept one explicit PNG/JPEG only；repair_plan never executes arbitrary scripts；do not report source path/name or commit source images, AI/SVG/PNG/JSON outputs, private `.ai`, fonts, or brushes |
