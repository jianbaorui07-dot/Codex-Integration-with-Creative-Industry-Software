...
| [artokun/comfyui-mcp](https://github.com/artokun/comfyui-mcp) | workflow 可视化/组合、WebSocket 进度、完成通知、持久化历史与 debug hooks | 已有 compose/validate/repair/lifecycle/visualize；仍缺实时进度与可恢复历史 |
| [IO-AtelierTech/comfyui-mcp](https://github.com/IO-AtelierTech/comfyui-mcp) | queue/history/interrupt/object_info 等完整作业控制面 | `agent_run` 已有安全门；仍缺只读 queue snapshot、受控 cancel 与 history 摘要 |
