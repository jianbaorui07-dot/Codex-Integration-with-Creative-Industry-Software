...
| [artokun/comfyui-mcp](https://github.com/artokun/comfyui-mcp) | queue 默认省略 workflow、WebSocket 实时进度、完成通知和 VRAM watchdog | 已实现更严格的脱敏 queue snapshot 与 live progress/stalled 状态层；仍缺断线恢复、完成通知和 VRAM guard |
...
| P1 | 只读 queue snapshot + live 结构化进度 | 支撑长任务监控，不需要先开放生成或写入 | 已实现 `comfyui.queue_snapshot` v1 与 `comfyui.progress_monitor` v1：默认 plan-only、live 仅 loopback、逻辑 job/node ID、单调进度、终态与 stalled；断线恢复仍待实现 |
