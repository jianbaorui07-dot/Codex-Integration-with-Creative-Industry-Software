...
本工具不读取 `/history`、不打开 WebSocket、不发送 MCP `notifications/progress`，也不提供 cancel / clear / move。实时事件由独立的 `comfyui.progress_monitor` 处理，见 [ComfyUI 实时进度监控](comfyui-progress-monitor.md)；它不会扩大 queue snapshot 的权限。
