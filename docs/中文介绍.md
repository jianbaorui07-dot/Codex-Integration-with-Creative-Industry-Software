...
星桥三联是一套面向本地创作工作站的接入方案。它把 Codex、GitHub/Jules、ComfyUI、Blender 5.0、CAD、Photoshop 和 Illustrator 放在同一条可追踪的工作链里，让 Codex 负责写脚本、整理流程、调用本地接口和记录结果，让专业软件继续负责各自擅长的创作任务。
...
- **Illustrator** 负责矢量图形、图标、版式草图、SVG/PDF 导出和可编辑图稿初稿。
...
| `docs/05-codex-illustrator.md` | 5. Codex 接入 Illustrator 中文介绍 |
| `examples/bridge_status.py` | 一次检查 ComfyUI、Blender、CAD、Photoshop、Illustrator 的本机状态 |
...
flowchart LR
  ...
  Codex --> Illustrator["Illustrator COM / JavaScript"]
  Comfy --> Images["本地图像输出"]
  Blender --> Scenes["本地三维场景"]
  CAD --> Drawings["本地 DWG 图纸"]
  PS --> Retouch["本地修图输出"]
  Illustrator --> Vectors["本地矢量输出"]
```
...
### 5. Codex x Illustrator：矢量设计桥

Illustrator 是矢量图形和版式设计引擎。Codex 可以通过 Windows COM 调用 Illustrator JavaScript，完成最小接入：连接已打开的 Illustrator、创建测试画板、绘制基础矢量对象并导出 PNG。

已经具备：

- `docs/05-codex-illustrator.md`：Illustrator 本地桥中文介绍和安全边界。
- `examples/illustrator_bridge/README.md`：运行说明和边界。
- `examples/illustrator_bridge/scripts/com_probe.ps1`：创建测试画板并导出 PNG。

适合继续扩展：

- 读取当前文档、画板、选中对象和图层状态。
- 根据结构化参数生成图标、简单版式、SVG 草图和品牌物料初稿。
- 增加 SVG/PDF 导出示例，输出路径由参数传入。
- 把稳定动作封装成 MCP 工具，让 Codex 能调用 `get_document_info`、`create_vector_probe`、`export_png` 等安全动作。
...
| 设计稿和矢量设计 | Illustrator COM、Penpot MCP、Figma MCP | 中 |
...
- 继续优化 Illustrator COM 探针、当前文档只读状态、SVG/PDF 导出和 MCP 封装，所有个人路径和素材信息只留本机。
- 为 Penpot/Figma、Krita 建立候选接入评估表，先记录来源、许可、依赖、账号要求和安全边界，再决定是否下载或安装。
- 逐步把稳定脚本包装成 MCP 工具，让 Codex 能稳定调用、记录结果并提示风险。

## 结论

星桥三联是一套本地创作工作台的组织方式：Codex 负责把需求拆成脚本和流程，GitHub 保存可公开协作的工程说明，Jules 做异步只读审查，ComfyUI 生成图像，Blender 构建三维，CAD 输出精确图纸，Photoshop 处理修图和抠图，Illustrator 处理矢量设计和导出。每条桥保持清楚边界，既提升创作效率，也让本地资产和账号安全保持可控。
