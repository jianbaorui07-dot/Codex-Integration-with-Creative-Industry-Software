...
- **彩色矢量化有双路线**：受控原生 Illustrator Image Trace 提供 plan / validate / compare / repair_plan / execute，其中 repair_plan 只生成最多 3 轮的白名单参数与默认 dry-run 的 execute/compare 安全模板，不自动执行；headless fallback 可离线生成背景 `<rect>` + 可编辑 `<path>` 的 SVG、预览和脱敏产物清单。两者证据分开报告。
...
| Illustrator / AI 矢量文件 | `illustrator.color_vectorize_*` 原生路线、环境探测、headless 彩色位图→已验证 SVG fallback | headless 只需 Python `.[illustrator-trace]`；原生路线需 Windows、已授权 Illustrator、PowerShell、COM | headless 不保证复杂照片、渐变、透明度或文字保真；原生路线未用公开授权样例做桌面 E2E | 接入显式确认的有界 repair 编排；评估 VTracer、最终 SVG 回栅格差异和原生桌面 E2E |
...
`status:manifest` 读取 `examples/*_bridge/bridge.json`，表示仓库配置状态。`status:probe` 探测本机环境，可能因为本机没有安装或没有打开专业软件而显示 `warn` / `missing`。`comfy:txt2img` 会执行真实 ComfyUI 任务，必须先启动 ComfyUI，并显式传入 `--ckpt`。`illustrator:vectorize:offline` 不启动 Illustrator，只接受一个显式输入并写入被 Git 忽略的安全输出目录；源图与输出都不能提交。
