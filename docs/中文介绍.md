...
| --- | --- | --- | --- | --- |
| ComfyUI | `comfyui.system_probe`、`comfyui.queue_snapshot`、`comfyui.progress_monitor`、`comfyui.workflow_validate`、`comfy.workflow_lifecycle_summary`、txt2img API 示例 | Python 3.10+；本机 ComfyUI server；live progress 需 `.[comfy]` 可选依赖 | 不能提交模型或生成图；实时监控不返回原始事件、预览或异常正文 | 补断线恢复、受控 cancel 和 queue payload dry-run |
