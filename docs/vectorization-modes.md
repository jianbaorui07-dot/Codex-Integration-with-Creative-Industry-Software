...
| --- | --- | --- | --- |
| `artisan` 匠心矢量 | 16 色、自适应锚点、设计分层；细线稿可切换为质量门控中心线描边 | 艺术稿、品牌图形、传统纹样、高级设计交付 | 否；受轮廓、面积和中心线栅格质量门槛约束 |
...
- 匠心模式额外使用曲线拟合、设计角色层级和线稿自适应。细线稿中心线只有在锚点减少、像素精确率、召回率、Dice 与复杂度同时达标时才启用，否则保留轮廓填充结果。当前角色来自本地构图推断，不宣称已经识别人脸、文字或具体物体语义。
