...
| Path | Purpose |
| --- | --- |
| `ue_project/Content/Amara/` | UE maps, blueprints, materials, UI, and editor utility assets created for the Amara sandbox. |
| `ue_project/Project8_Amara.uproject` | Minimal sanitized project descriptor copied from the enhanced UE project. |
| `control_layer/` | Offline control scripts, task schemas, task JSON, and sanitized execution evidence. |
| `tasks_and_checkpoints/` | Plans and checkpoint JSON used by the Amara offline workflow. |
...
- Amara UE assets under `Content/Amara`.
...
- `M_AmaraLab` and `M_AmaraBlockoutSandbox`.
- roughly 42 Amara-managed actors after the city upgrade task.
