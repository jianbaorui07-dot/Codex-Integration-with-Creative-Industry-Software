...
- AMARA_ROOT：<AMARA_ROOT>
- AMARA_ROOT 存在：True
- 本次新建审查目录：<AMARA_ROOT>\00_审查与基线
- 现有增强副本只读清单：<AMARA_ROOT>\00_审查与基线\现有增强副本只读清单_20260705_141729.csv
...
- 备份路径：<AMARA_ROOT>\01_项目8原始备份
- 备份存在：True
- 备份文件数：75
- 备份只读文件数：75
- 原始项目与备份哈希差异数：0
- 当前既有增强副本路径：<AMARA_ROOT>\02_项目8_Codex增强版
- 当前既有增强副本存在：True
- 当前既有增强副本文件数：77
- 本次目标 Amara 增强副本路径：<AMARA_ROOT>\02_项目8_Amara增强版
- 本次目标 Amara 增强副本存在：False
- 说明：现有增强副本名为 02_项目8_Codex增强版，不是目标要求的 02_项目8_Amara增强版。后续不得覆盖现有增强副本；若严格执行目标路径，应新建或定位 02_项目8_Amara增强版 后再启动 UE。

## 4. 当前已有 AMARA_ROOT 顶层结构
...
- <AMARA_ROOT>\99_临时文件\legacy_root_files_20260705_132931\Codex_UE_首次测试报告.md | 2026-07-04T22:01:09 | 1578 bytes
- <AMARA_ROOT>\99_临时文件\legacy_root_files_20260705_132931\UE_Codex_MCP_接入前审查报告.md | 2026-07-04T22:14:30 | 7778 bytes
- <AMARA_ROOT>\00_审查与日志\项目8接入前审查报告.md | 2026-07-05T13:35:11 | 2055 bytes
- <AMARA_ROOT>\00_审查与日志\项目8备份与增强副本复制报告_20260705_133559.md | 2026-07-05T13:35:59 | 7758 bytes
- <AMARA_ROOT>\00_审查与日志\项目8备份与增强副本复制报告_20260705_133650_corrected.md | 2026-07-05T13:36:52 | 667 bytes
- <AMARA_ROOT>\05_报告与说明\UE_Codex接入与项目8增强完成报告.md | 2026-07-05T13:49:43 | 6446 bytes
...
- 备份文件数：75
- 差异数：0
- 结论：原始项目8与 AMARA_ROOT 备份当前一致，未发现哈希差异。
...
- 结论：未发现 Content\Amara 或 Content\CodexIntegration；没有证据显示增强副本已创建 Amara 框架内容。

## 8. ue_python_api_probe.py 只读审查

- 存在：<AMARA_ROOT>\99_临时文件\ue_python_api_probe.py
...
- 会创建 OUT_DIR；默认位于 AMARA_ROOT\99_临时文件，属于允许写入范围
- 会写入 ue_python_api_probe.json；默认位于 AMARA_ROOT\99_临时文件


审查结论：ue_python_api_probe.py 可判定为低风险探针脚本：不访问外网、不安装软件、不执行 shell、不删除文件、不监听网络；如将来执行，会在 AMARA_ROOT 的 99_临时文件 下写入一个 JSON 文件。未经后续阶段需要，不在本阶段执行。

## 9. 已存在风险

- 上次 UE 默认 UdpMessaging 曾出现 0.0.0.0、组播地址和局域网地址；这是继续流程的最高优先级风险。
- 当前目标要求的 02_项目8_Amara增强版 尚不存在，而旧增强副本为 02_项目8_Codex增强版；后续必须避免覆盖旧增强副本。
- 原项目是纯蓝图项目，无 Source/Plugins/Modules；不得创建 C++、Native Plugin 或编译路径。
- 原项目已有 Content\CodexTests 测试内容，这是既有状态；本次 Amara 内容不得修改旧内容。
...
3. 若需要严格满足本次目标路径，先在 AMARA_ROOT 内创建新的 02_项目8_Amara增强版 或确认已有目标路径后再启动 UE。
