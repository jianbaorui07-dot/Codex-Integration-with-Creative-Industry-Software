...
def warn(msg):
    ...
    unreal.log_warning("Amara Offline Bridge warning: %s" % msg)


def err(label, exc):
    RESULT["errors"].append({"label": label, "error": str(exc), "traceback": traceback.format_exc()})
    unreal.log_error("Amara Offline Bridge error in %s: %s" % (label, exc))
...
def load_task():
    task_path = path_in_root("03_Amara控制层", "Tasks", "Inbox", "002_未来城市漫游升级.json")
...
def validate_task(task):
    ...
    strict = {
        ...
        "allow_network": False,
        "allow_delete_non_amara_actor": False,
        "pie_test_seconds": 20,
...
def create_blueprint(name, parent_class, role):
    ...
    if bp:
        unreal.EditorAssetLibrary.set_metadata_tag(bp, "AmaraRole", role)
        unreal.EditorAssetLibrary.set_metadata_tag(bp, "AmaraOfflineBridge", "v0.1")
        RESULT["created_assets"].append(path)
        save_asset(path)
    return bp
...
def create_euw_or_widget():
    package = "/Game/Amara/EditorTools"
    name = "EUW_AmaraControlDesk"
    path = package + "/" + name
    if has_asset(path):
        return unreal.EditorAssetLibrary.load_asset(path)
    ...
    try:
        if checks.get("EditorUtilityWidgetBlueprint") and checks.get("EditorUtilityWidgetBlueprintFactory"):
            ...
            warn("Editor Utility Widget class unavailable; created WidgetBlueprint fallback named EUW_AmaraControlDesk")
        if asset:
            unreal.EditorAssetLibrary.set_metadata_tag(asset, "AmaraRole", "Offline control desk: read task, preview, checkpoint, execute, summary, 20s PIE, restore, stop")
            RESULT["created_assets"].append(path)
            save_asset(path)
            return asset
    except Exception as exc:
...
def create_upgrade(materials):
    ...
    spawn_actor(unreal.StaticMeshActor, "AMARA_UPG_PlazaLightStrip_A", unreal.Vector(-260, -650, 18), scale=unreal.Vector(0.22, 6.2, 0.035), mesh=cube, material=materials["light"])
    spawn_actor(unreal.StaticMeshActor, "AMARA_UPG_PlazaLightStrip_B", unreal.Vector(260, -650, 18), scale=unreal.Vector(0.22, 6.2, 0.035), mesh=cube, material=materials["light"])
    # 4 window bands on existing building groups
    spawn_actor(unreal.StaticMeshActor, "AMARA_UPG_WindowBand_L01", unreal.Vector(-520, -520, 175), scale=unreal.Vector(2.82, 0.06, 0.18), mesh=cube, material=materials["window"])
    spawn_actor(unreal.StaticMeshActor, "AMARA_UPG_WindowBand_L02", unreal.Vector(-520, 280, 220), scale=unreal.Vector(2.82, 0.06, 0.18), mesh=cube, material=materials["window"])
    spawn_actor(unreal.StaticMeshActor, "AMARA_UPG_WindowBand_R01", unreal.Vector(520, -520, 175), scale=unreal.Vector(2.82, 0.06, 0.18), mesh=cube, material=materials["window"])
    spawn_actor(unreal.StaticMeshActor, "AMARA_UPG_WindowBand_R02", unreal.Vector(520, 280, 220), scale=unreal.Vector(2.82, 0.06, 0.18), mesh=cube, material=materials["window"])
    # tower beacon
    beacon = spawn_actor(unreal.PointLight, "AMARA_UPG_TowerBeacon", unreal.Vector(0, 320, 890))
    if beacon:
        light_intensity(beacon, 950.0)
    # guide lines
    spawn_actor(unreal.StaticMeshActor, "AMARA_UPG_GuideLine_L", unreal.Vector(-90, -260, 16), scale=unreal.Vector(0.08, 16, 0.025), mesh=cube, material=materials["guide"])
    spawn_actor(unreal.StaticMeshActor, "AMARA_UPG_GuideLine_R", unreal.Vector(90, -260, 16), scale=unreal.Vector(0.08, 16, 0.025), mesh=cube, material=materials["guide"])
    # zone markers
    sign1 = spawn_actor(unreal.TextRenderActor, "AMARA_UPG_ZoneMarker_Plaza", unreal.Vector(-360, -950, 80), unreal.Rotator(0, 0, 20))
    if sign1:
        sign1.text_render.set_editor_property("text", "PLAZA")
        sign1.text_render.set_editor_property("world_size", 38.0)
    sign2 = spawn_actor(unreal.TextRenderActor, "AMARA_UPG_ZoneMarker_Tower", unreal.Vector(210, 120, 120), unreal.Rotator(0, 0, -25))
    if sign2:
        sign2.text_render.set_editor_property("text", "TOWER")
        sign2.text_render.set_editor_property("world_size", 42.0)
    # observation point
    spawn_actor(unreal.StaticMeshActor, "AMARA_UPG_ObservationPoint", unreal.Vector(-240, -1150, 42), scale=unreal.Vector(1.2, 1.2, 0.18), mesh=cube, material=materials["observation"])
    # explorer pawn and mood controller instances
    pawn_bp = unreal.EditorAssetLibrary.load_asset("/Game/Amara/Blueprints/BP_AmaraExplorerPawn")
    pawn_class = pawn_bp.generated_class() if pawn_bp else unreal.DefaultPawn
    pawn = spawn_actor(pawn_class, "AMARA_UPG_ExplorerPawn", unreal.Vector(-240, -1150, 120), unreal.Rotator(0, 0, 0))
    if pawn:
        try:
    ...
    mood_bp = unreal.EditorAssetLibrary.load_asset("/Game/Amara/Blueprints/BP_AmaraCityMoodController")
    mood_class = mood_bp.generated_class() if mood_bp else unreal.Actor
    spawn_actor(mood_class, "AMARA_UPG_MoodController", unreal.Vector(0, -850, 120))
...
def create_assets():
    ...
    create_blueprint("BP_AmaraOfflineTaskRunner", unreal.Actor, "Reads whitelisted offline tasks and writes checkpoints/world summaries without network or deletes.")
    create_blueprint("BP_AmaraCityMoodController", unreal.Actor, "Controls Amara day/night preview state for Amara lights/materials only; no global render settings.")
    create_blueprint("BP_AmaraExplorerPawn", unreal.DefaultPawn, "DefaultPawn-derived lightweight explorer pawn; inherits basic movement and mouse look; placed with AutoPossess Player0.")
    mats = {
        "light": create_material("M_Amara_Upgrade_LightStrip", unreal.LinearColor(0.05, 0.22, 0.28, 1), unreal.LinearColor(0.0, 0.75, 1.0, 1), 2.0),
        "window": create_material("M_Amara_Upgrade_WindowGlow", unreal.LinearColor(0.04, 0.08, 0.12, 1), unreal.LinearColor(0.3, 0.95, 1.0, 1), 1.4),
        "guide": create_material("M_Amara_Upgrade_GuideLine", unreal.LinearColor(0.02, 0.12, 0.14, 1), unreal.LinearColor(0.0, 0.55, 0.95, 1), 1.1),
        "observation": create_material("M_Amara_Upgrade_Observation", unreal.LinearColor(0.18, 0.22, 0.20, 1), unreal.LinearColor(0.12, 0.5, 0.35, 1), 0.5),
    }
    return mats
...
def preview_plan(pre_summary):
    ...
    RESULT["preview"] = {
        ...
        "pre_amara_actor_count": pre_summary.get("amara_actor_count"),
        "post_amara_actor_estimate": pre_summary.get("amara_actor_count", 0) + len(to_add),
        "within_build_limit_per_step": len(to_add) <= 15,
        "within_actor_limit": pre_summary.get("amara_actor_count", 0) + len(to_add) <= 60,
...
def write_reports(pre_cp, post_cp):
    report_dir = path_in_root("03_Amara控制层", "Logs")
    ensure_dir(report_dir)
    path = os.path.join(report_dir, "Amara_OfflineBridge_任务002执行结果.json")
    RESULT["pre_checkpoint"] = pre_cp
    RESULT["post_checkpoint"] = post_cp
    with open(path, "w", encoding="utf-8") as f:
        json.dump(RESULT, f, ensure_ascii=False, indent=2)
    md_path = os.path.join(report_dir, "Amara_OfflineBridge_任务002执行摘要.md")
    with open(md_path, "w", encoding="utf-8") as f:
        f.write("# Amara Offline Bridge 任务002执行摘要\n\n")
        f.write("- 状态：%s\n" % RESULT["status"])
        f.write("- 新增 Actor：%s\n" % len(RESULT["created_actors"]))
        f.write("- 升级前 Amara Actor：%s\n" % RESULT["pre_summary"].get("amara_actor_count"))
        f.write("- 升级后 Amara Actor：%s\n" % RESULT["post_summary"].get("amara_actor_count"))
...
def main():
    try:
        task = load_task()
        unreal.EditorLevelLibrary.load_level("/Game/Amara/Maps/M_AmaraBlockoutSandbox")
        pre = world_summary()
        ...
        if post_mid.get("amara_actor_count", 9999) > 60:
            raise RuntimeError("Amara actor count exceeded 60 after upgrade")
        ...
        for actor in unreal.EditorLevelLibrary.get_all_level_actors():
            if actor.get_actor_label() == "AMARA_UPG_TowerBeacon":
