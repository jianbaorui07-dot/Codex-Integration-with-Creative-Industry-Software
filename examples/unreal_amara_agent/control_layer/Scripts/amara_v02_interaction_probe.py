...
MAP_PATH = "/Game/Amara/Maps/M_AmaraBlockoutSandbox"
TAG = "AmaraManaged"
RESULT = {
    "status": "started",
    "errors": [],
...
def out_path():
    return os.path.join(
        ...
        "03_Amara控制层",
        "Logs",
        "Amara_v02_interaction_probe.json",
    )


def warn(message):
    RESULT["warnings"].append(message)
    unreal.log_warning("Amara v0.2 probe warning: %s" % message)
...
def err(label, exc):
    ...
    unreal.log_error("Amara v0.2 probe error in %s: %s" % (label, exc))
...
def collect_blueprints():
    paths = [
        ...
        "/Game/Amara/Blueprints/BP_AmaraCheckpointManager",
        "/Game/Amara/EditorTools/EUW_AmaraControlDesk",
        "/Game/Amara/UI/WBP_AmaraStatus",
    ]
    for path in paths:
...
def write_result():
    try:
        ...
        unreal.log("Amara v0.2 interaction probe wrote %s" % out_path())
    except Exception as exc:
        unreal.log_error("Amara v0.2 probe failed to write result: %s" % exc)
