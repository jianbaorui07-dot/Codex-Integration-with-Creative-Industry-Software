...
def out_path(*parts):
    return os.path.join(amara_root(), *parts)
...
def record_error(label, exc):
    ...
    unreal.log_error("Amara error in %s: %s" % (label, exc))


def record_warning(msg):
    RESULT["warnings"].append(msg)
    unreal.log_warning("Amara warning: %s" % msg)
...
def make_content_dirs():
    dirs = [
        "/Game/Amara",
        "/Game/Amara/Blueprints",
        "/Game/Amara/Maps",
        "/Game/Amara/Materials",
        "/Game/Amara/RemoteControl",
        "/Game/Amara/UI",
        "/Game/Amara/Data",
        "/Game/Amara/Checkpoints",
        "/Game/Amara/Tests",
        "/Game/Amara/Docs",
    ]
    for d in dirs:
...
def create_blueprint(name, role_text):
    ...
    try:
        ...
        if bp:
            unreal.EditorAssetLibrary.set_metadata_tag(bp, "AmaraRole", role_text)
            unreal.EditorAssetLibrary.set_metadata_tag(bp, "AmaraAllowedStates", ",".join(STATE_VALUES))
            unreal.EditorAssetLibrary.set_metadata_tag(bp, "AmaraCommandWhitelist", ",".join(COMMAND_WHITELIST))
            RESULT["created_assets"].append(asset_path)
            save_asset(asset_path)
            return bp
    except Exception as exc:
...
def create_widget_blueprint():
    package = "/Game/Amara/UI"
    name = "WBP_AmaraStatus"
    asset_path = package + "/" + name
    if asset_exists(asset_path):
        return unreal.EditorAssetLibrary.load_asset(asset_path)
    try:
        ...
        if wb:
            unreal.EditorAssetLibrary.set_metadata_tag(wb, "AmaraRole", "Status display only: map, task, state, actor count, checkpoint, safety state")
            RESULT["created_assets"].append(asset_path)
            save_asset(asset_path)
            return wb
    except Exception as exc:
...
def create_remote_control_preset():
    package = "/Game/Amara/RemoteControl"
    name = "RC_AmaraAgentFramework"
    asset_path = package + "/" + name
    if asset_exists(asset_path):
        return unreal.EditorAssetLibrary.load_asset(asset_path)
    try:
        ...
        if preset:
            unreal.EditorAssetLibrary.set_metadata_tag(preset, "AmaraExposedTools", "HealthCheck,GetWorldSummary,PrintSmokeMessage,SetTestLightRotation,CreateCheckpoint,RestoreLastCheckpoint,PreviewApprovedPlan,ExecuteApprovedPlan,StopCurrentTask")
            RESULT["created_assets"].append(asset_path)
            save_asset(asset_path)
            return preset
    except Exception as exc:
...
def create_lab_map(materials):
    path = "/Game/Amara/Maps/M_AmaraLab"
    if asset_exists(path):
        record_warning("M_AmaraLab already exists; loading without overwrite")
    ...
    spawn(unreal.StaticMeshActor, "AMARA_LAB_Ground", unreal.Vector(0, 0, -10), scale=unreal.Vector(20, 20, 0.1), mesh=cube_mesh, material=materials.get("ground"))
    spawn(unreal.StaticMeshActor, "AMARA_LAB_TestCube", unreal.Vector(0, 0, 100), scale=unreal.Vector(1, 1, 1), mesh=cube_mesh, material=materials.get("accent"))
    sun = spawn(unreal.DirectionalLight, "AMARA_LAB_DirectionalLight", unreal.Vector(-300, -300, 500), unreal.Rotator(-35, -30, 0))
    set_light_intensity(sun, 2.0)
    spawn(unreal.SkyLight, "AMARA_LAB_SkyLight", unreal.Vector(0, 0, 250))
    spawn(unreal.PlayerStart, "AMARA_LAB_PlayerStart", unreal.Vector(-300, 0, 80))
    spawn(unreal.CameraActor, "AMARA_LAB_Camera", unreal.Vector(-650, -500, 360), unreal.Rotator(-25, 0, -38))
    try:
        txt = spawn(unreal.TextRenderActor, "AMARA_LAB_StatusText", unreal.Vector(-240, 0, 180), unreal.Rotator(0, 0, 0))
        txt.text_render.set_editor_property("text", "AMARA UE AGENT FRAMEWORK v0.1")
        txt.text_render.set_editor_property("world_size", 26.0)
    except Exception as exc:
        record_warning("status text unavailable: %s" % exc)
    save_current_level("M_AmaraLab")


def create_blockout_map(materials):
    path = "/Game/Amara/Maps/M_AmaraBlockoutSandbox"
    if asset_exists(path):
        record_warning("M_AmaraBlockoutSandbox already exists; loading without overwrite")
        unreal.EditorLevelLibrary.load_level(path)
        return
    unreal.EditorLevelLibrary.new_level(path)
    cube_mesh = unreal.EditorAssetLibrary.load_asset("/Engine/BasicShapes/Cube.Cube")
    spawn(unreal.StaticMeshActor, "AMARA_CITY_Ground", unreal.Vector(0, 0, -10), scale=unreal.Vector(60, 40, 0.1), mesh=cube_mesh, material=materials.get("ground"))
    spawn(unreal.StaticMeshActor, "AMARA_CITY_MainRoad", unreal.Vector(0, 0, 0), scale=unreal.Vector(4, 38, 0.04), mesh=cube_mesh, material=materials.get("road"))
    spawn(unreal.StaticMeshActor, "AMARA_CITY_Plaza", unreal.Vector(0, -650, 5), scale=unreal.Vector(12, 8, 0.05), mesh=cube_mesh, material=materials.get("plaza"))
    spawn(unreal.StaticMeshActor, "AMARA_CITY_CenterTower", unreal.Vector(0, 320, 420), scale=unreal.Vector(2.2, 2.2, 8.4), mesh=cube_mesh, material=materials.get("tower"))
    idx = 0
    for side, x in [("L", -520), ("R", 520)]:
        for row, y in enumerate([-900, -520, -120, 280, 700, 1060]):
            height = 1.8 + (row % 3) * 0.9
            idx += 1
            spawn(unreal.StaticMeshActor, "AMARA_CITY_Building_%s_%02d" % (side, idx), unreal.Vector(x, y, height * 50), scale=unreal.Vector(2.8, 2.1, height), mesh=cube_mesh, material=materials.get("building"))
    for y in [-1050, -750, -450, -150, 150, 450, 750, 1050]:
        light = spawn(unreal.PointLight, "AMARA_CITY_StreetLight_%s" % y, unreal.Vector(230, y, 130))
        set_light_intensity(light, 450.0)
    sun = spawn(unreal.DirectionalLight, "AMARA_CITY_MoonKeyLight", unreal.Vector(-700, -900, 900), unreal.Rotator(-18, -45, 0))
    set_light_intensity(sun, 0.35)
    sky = spawn(unreal.SkyLight, "AMARA_CITY_SkyLight", unreal.Vector(0, 0, 300))
    set_light_intensity(sky, 0.15)
    spawn(unreal.PlayerStart, "AMARA_CITY_PlayerStart", unreal.Vector(-250, -1050, 90))
    spawn(unreal.CameraActor, "AMARA_CITY_OverviewCamera", unreal.Vector(-1350, -1450, 950), unreal.Rotator(-35, 0, -42))
    save_current_level("M_AmaraBlockoutSandbox")
...
def world_summary():
    ...
    return {
        ...
        "amara_actor_count": len(amara),
        "allowed_to_continue": len(amara) <= 100,
    }
...
def main():
    ...
    RESULT["amara_root"] = amara_root()
    make_content_dirs()
    materials = {
        "ground": create_material("M_Amara_Ground", unreal.LinearColor(0.12, 0.14, 0.12, 1.0)),
        "road": create_material("M_Amara_Road", unreal.LinearColor(0.035, 0.038, 0.045, 1.0)),
        "plaza": create_material("M_Amara_Plaza", unreal.LinearColor(0.22, 0.22, 0.18, 1.0)),
        "tower": create_material("M_Amara_Tower", unreal.LinearColor(0.08, 0.22, 0.34, 1.0)),
        "building": create_material("M_Amara_Building", unreal.LinearColor(0.16, 0.17, 0.20, 1.0)),
        "accent": create_material("M_Amara_Accent", unreal.LinearColor(0.20, 0.55, 0.85, 1.0)),
    }
    create_blueprint("BP_AmaraAgentDirector", "Tracks task name, status, execution stage, scene summary, last checkpoint, failure reason, and resource budget.")
    create_blueprint("BP_AmaraSafetyController", "Guards write operations, actor limits, AmaraManaged-only actions, and Content/Amara-only asset scope.")
    create_blueprint("BP_AmaraCheckpointManager", "Creates and restores AmaraManaged actor checkpoints without touching old content.")
    create_blueprint("BP_AmaraWorldProbe", "Reports map, Amara actor count, light rotation, task state, checkpoint, continuation status, and budget status.")
    create_blueprint("BP_AmaraCommandRouter", "Accepts only the Amara command whitelist and rejects arbitrary natural-language execution.")
    create_blueprint("BP_AmaraBlockoutBuilder", "Builds approved cube/plane/light blockouts within actor budget.")
    ...
    if RESULT["world_summary_after_build"].get("amara_actor_count", 9999) <= 100:
    ...
    result_path = os.path.join(result_dir, "amara_ue_build_result.json")
    with open(result_path, "w", encoding="utf-8") as f:
        json.dump(RESULT, f, ensure_ascii=False, indent=2)
    unreal.log("Amara build result written to %s" % result_path)
