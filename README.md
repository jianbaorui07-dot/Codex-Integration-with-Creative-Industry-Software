...
普通图片默认进入**智能矢量**；Logo、图标和纹样可选择**轻量矢量**；需要技术验证或像素存档时选择**精确重建**。在三种基础模式之上，新增加定位更高的 **匠心矢量**：保留关键角点，以更少锚点生成直线、三次贝塞尔轮廓和人工描线式开放描边，并把基础、主体、细节和点睛形状组织为可引用的设计图层。所有模式都生成纯路径 SVG，并拒绝嵌入位图、脚本和外链；均不调用 Illustrator Image Trace。
...
| 模式 | 产品定位 | 核心处理 |
| --- | --- | --- |
| **匠心矢量（高级）** | 艺术稿、品牌图形和接近人工绘制的高级交付 | 自适应少锚点、角点保护、三次贝塞尔、设计图层、中心线描边、质量门控与安全回退 |
